...
$triggers = @(
    ...
    # general-engineering-routing removed (consolidated into ecc:* + engineering:*)
    ...
    # === Compliance / audit ===
    # Slimmed: only smart contract + automation governance. SOC 2 / ISO / HIPAA → operations:compliance-tracking + anthropic-skills:security-audit-engine
    @{ Pattern = '(?i)\b(smart contract audit|blockchain audit|gas optimization audit|DeFi audit|reentrancy|oracle manipulation|n8n governance|automation governance|workflow audit at scale)\b'; Agent = ''; Skill = 'compliance-audit-routing' }
    ...
    # Slimmed: Zettelkasten + enterprise training only. Document formats → anthropic-skills:pptx/docx/pdf/xlsx
    @{ Pattern = '(?i)\b(Zettelkasten|Luhmann|atomic notes|connectivity validation|knowledge graph stewardship|enterprise training curriculum|internal trainer development|blended learning program)\b'; Agent = ''; Skill = 'knowledge-content-routing' }
    # ai-infra-routing removed (consolidated into ecc:mcp-server-patterns, ecc:autonomous-agent-harness, anthropic-skills:mcp-builder, ruflo-swarm:*)
    ...
    # design-routing removed (consolidated into design:* skills)
    # Slimmed: behavioral-nudge + trend-researcher persona only. Sprint/PRD/roadmap → product-management:*
    @{ Pattern = '(?i)\b(behavioral nudge|habit loop architecture|motivational design|choice architecture|nudge audit|trend researcher persona)\b'; Agent = ''; Skill = 'product-discovery-routing' }
